 ...
 */
 ...
 public class Main{
	 ...
	 */
	 ...
	 /**
	  * Handles user input for navigating back to the previous menu or performing 
	  * a specific operation. The user can enter 'r' or 'R' to return to the 
	 ...
	 public static void choicee(int choice)
	 {
		 ...
		 switch (choice)
		 {
			 case 1: // Case to encrypting the message
				 ...
				 while (!shiftVal) //while loop to ask for a value again in case of entering avalue outside the desired range 
				 {
						   
					 System.out.print("Enter a 'shift' value between -26 and 26 (include): ");
					 
					 if(input.hasNextInt()) //Checks if the value entered is an integer
					 {
						 ...
						 if(shiftValue >= -26 && shiftValue <= 26) 
						 {
							 ...
							 shiftVal = true; //Breaks the loop
						 }
						 else 
						 {
							System.out.println("The 'shift' value should be in the [-26,26] range."); //If the value is bigger than 26 or smaller than -26
						 }
					 }
					 else
					 {
						System.out.println("Invalid input. Please enter an integer."); //If the value is not an integer
						input.next();
					 }	  
				 }
			 ...
			 case 2: // Case to decrypting the message
				 ...
				 {		  
					 ...
					 if (input.hasNextInt()) //Checks if the value entered is an integer
					 {
						 ...
						 {
							//If the value is bigger than 26 or smaller than -26
				 ...
				 //To convert encryption into decryption when calling the method shiftValue is subtracted from 26 which is size of the range.
